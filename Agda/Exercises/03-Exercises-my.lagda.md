...
is-bijection' {A} {B} f = Σ f⁻¹ ꞉ (B → A) , (f⁻¹ ∘ f ∼ id) × (f ∘ f⁻¹ ∼ id)
 ...
 where
  ...
  gf (suc n) (suc k) = γ
   where
    ...
    γ = g (suc n) (f (suc n) (suc k)) ≡⟨ ap (g (suc n)) (refl (suc' (f n k))) ⟩
  ...
  fg (suc n) (inr k) = γ
   where
    ...
    γ = f (suc n) (g (suc n) (suc' k)) ≡⟨ ap (f (suc n)) (refl (suc (g n k))) ⟩
        f (suc n) (suc (g n k))        ≡⟨ refl (suc' (f n (g n k))) ⟩
...
zero-minimal-element : {P : ℕ → Type} → P 0 → minimal-element P
zero-minimal-element p0 = zero , p0 , λ _ _ → ⋆
...
well-ordering-principle-suc P d n p (inl p0) _  = zero-minimal-element p0
...
```agda
well-ordering-principle : (P : ℕ → Type) → (d : is-decidable-predicate P) → (n : ℕ) → P n → minimal-element P
well-ordering-principle P d 0 p = zero-minimal-element p
well-ordering-principle P d (suc n) p = well-ordering-principle-suc P d n p (d 0)
     -- {!well-ordering-principle-suc (λ z → P (suc z))!}
